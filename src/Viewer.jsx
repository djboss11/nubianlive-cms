...
const DEFAULT_CATEGORIES = [
  ...
  },
  {
    name: "Live Now",
    items: [
      { id: 10, title: "Eastern",      thumb: "📺", type: "LIVE", tag: "LIVE", description: "Eastern Time — ET",           logo: "https://assets.nubianlive.com/live_east.png"     },
      { id: 15, title: "Central",      thumb: "📺", type: "LIVE", tag: "LIVE", description: "Central Time — CT",           logo: "https://assets.nubianlive.com/live_central.png"  },
...
const channels = [
  { id: 1, name: "Eastern",      current: "Live Now", next: "Coming Up", status: "live", thumb: "📺", blockOffsetSec: 0,     displayOffsetHr: 0,  tzLabel: "ET",  logo: "https://assets.nubianlive.com/live_east.png"    },
  { id: 5, name: "Central",      current: "Live Now", next: "Coming Up", status: "live", thumb: "📺", blockOffsetSec: 3600,  displayOffsetHr: -1, tzLabel: "CT",  logo: "https://assets.nubianlive.com/live_central.png" },
...
function LiveTV({ t, initialChannelId }) {
  ...
  const handleFullscreen = () => {
    if (!document.fullscreenElement) {
      (playerContainerRef.current || videoRef.current)?.requestFullscreen().catch(() => {});
    } else {
...
{/* SoundCloud iframe — rendered inside radio UI when playing */}
      ...
      <div style={{ display: "flex", flexDirection: isMobile ? "column" : "row", gap: 20 }}>
        ...
        <div style={{ background: "#111", borderRadius: 12, overflow: "hidden", border: "1px solid var(--border)", flex: 1 }}>
          <div ref={playerContainerRef} style={{ position: "relative", background: "#000", aspectRatio: "16/9", width: "100%" }}>
            {isRadio ? (
              /* Radio UI */
              <div style={{
                aspectRatio: "16/9",
...
function PPVSection({ t, subscription, isOwner, onPlayTrailer }) {
  const w = useWindowWidth();
  const sidePad = w < 768 ? 16 : 48;
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg)" }}>
      ...
      <div style={{ position: "relative", height: heroH, background: "#000", overflow: "hidden" }}>
        ...
        {/* Gradient overlays */}
        <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to top, var(--bg) 0%, rgba(0,0,0,0.3) 50%, transparent 100%)" }} />
        <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to right, rgba(0,0,0,0.45) 0%, transparent 70%)" }} />
  ...
  return (
    <LangContext.Provider value={{ lang, t, setLang }}>
      ...
      <div style={{ minHeight: "100vh" }}>
        ...
        {page === "ppv" && (
          <div style={{ paddingTop: 80 }}>
            <PPVSection t={t} subscription={subscription} isOwner={isOwner} onPlayTrailer={(ev) => setPlaying({ title: `${ev.title} — Trailer`, hlsUrl: hls(ev.trailer_id), poster: ev.poster_filename ? (ev.poster_filename.startsWith("http") ? ev.poster_filename : `${R2}/${ev.poster_filename}`) : undefined })} />
          </div>
        )}
